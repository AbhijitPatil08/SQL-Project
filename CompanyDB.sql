...
Select e.EmployeeID, e.firstname, e.lastname, e.DepartmentID, d.DepartmentName from employee e 
join department d
...
select e.employeeid, concat(e.firstname, e.lastname) as Employee_name, d.departmentname, d.location from employee e
join department d
on e. departmentid = d.departmentid;
-- 4. Retrieve the total number of employees in each department
select d.departmentname, count(e.employeeid) as TotalEmployee from department d
left join employee e
on e.departmentid = d.departmentid
group by d.departmentname;

-- 5. Find the highest-paid employee in each department
SELECT d.DepartmentName, e.FirstName, e.LastName, e.Salary FROM Employee e
JOIN Department d
ON e.DepartmentID = d.DepartmentID
...
SELECT e.FirstName, e.LastName, e.DateOfJoining, d.DepartmentName FROM Employee e
JOIN Department d 
ON e.DepartmentID = d.DepartmentID
WHERE e.DateOfJoining > '2020-01-01';

-- 7. Show employees earning more than ₹70,000 and their respective departments
SELECT e.FirstName, e.LastName, e.Salary, d.DepartmentName FROM Employee e
JOIN Department d 
ON e.DepartmentID = d.DepartmentID
WHERE e.Salary > 70000;

-- 8. Retrieve employees working in the "Pune" office
SELECT e.FirstName, e.LastName, d.DepartmentName, d.Location FROM Employee e
JOIN Department d
ON e.DepartmentID = d.DepartmentID
...
JOIN Department d 
ON e.DepartmentID = d.DepartmentID
ORDER BY d.DepartmentName ASC, e.FirstName ASC;
