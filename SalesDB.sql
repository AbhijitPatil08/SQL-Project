...
-- 1-	What is the total sales amount for each category?
	select category, sum(sale_amount) as Total_sales from sales
	group by category;
...
select avg(sale_amount), product_name from sales
group by product_name
having count(*)>2;
...
select product_name, max(sale_amount)as total_sales_amount from sales
group by product_name
