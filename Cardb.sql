...
SELECT Brand, COUNT(*) AS TotalModels FROM Cars
GROUP BY Brand
...
SELECT *
FROM Cars
ORDER BY ABS(Price - (SELECT ROUND(AVG(Price), -2) FROM Cars))
LIMIT 5;
...
order by price asc
limit 1;
-- 8.	Retrieve  than two car modbrands with moreels in the dataset.
